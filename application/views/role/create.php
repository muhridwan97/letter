<form action="<?= site_url('master/role/save') ?>" method="POST" id="form-role">
                    ...
                    <?php foreach ($permissions as $permission): ?>
                        ...
                        <?php if($lastGroup != $module): ?>
                            ...
                            <div class="col-12 mt-3">
                                <hr>
                                <h4 class="mt-2">
                                    <span class="form-check mt-0">
                                        <label class="form-check-label">
                                            <input type="checkbox" class="check_all form-check-input"<?= set_checkbox('check_all_'.$lastGroupName, $lastGroupName); ?>
                                                   id="check_all_<?= $lastGroupName ?>" name="check_all_<?= $lastGroupName ?>" value="<?= $lastGroupName ?>">
                        ...
                        <div class="col-sm-3">
                            <div class="form-check mt-2">
                                <label class="form-check-label">
                                    <input type="checkbox" class="<?= $lastGroupName ?> form-check-input"<?= set_checkbox('permissions[]', $permission['id']); ?>
    ...
    <div class="card grid-margin">
        <div class="card-body d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-success" data-toggle="one-touch" data-touch-message="Saving...">Save Role</button>
        </div>
    </div>
</form>