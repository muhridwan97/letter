<form action="<?= site_url('skripsi/save') ?>" method="POST" enctype="multipart/form-data" id="form-skripsi">
    <?= _csrf() ?>
    <div class="card mb-3">
        <div class="card-body">
            ...
            <?php if(AuthorizationModel::isAuthorized(PERMISSION_ALL_ACCESS)!=true && !AuthorizationModel::isAuthorized(PERMISSION_SKRIPSI_VALIDATE)): ?>
                <input type="hidden" name="student" id="student" value="<?= UserModel::loginData('id_civitas') ?>">
            <?php else: ?>
            ...
            <div class="form-group">
                ...
                <input type="hidden" id="id_pembimbing" name="id_pembimbing" value="<?= set_value('id_pembimbing', $pembimbingId) ?>">
                <input type="text" class="form-control" id="pembimbing" name="pembimbing" readonly maxlength="50"
                        value="<?= set_value('pembimbing', $pembimbing) ?>" placeholder="Nama pembimbing">
