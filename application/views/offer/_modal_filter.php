        ...
        <div class="modal-content">
            <form action="<?= site_url(uri_string()) ?>" id="form-filter">
                        ...
                        <div class="col-8 col-sm-6">
                            <div class="form-group">
                                ...
                                <select class="custom-select" name="sort_by" id="sort_by" required>
                                    ...
                                    <option value="requisition"<?= set_select('sort_by', 'requisition', get_url_param('sort_by') == 'requisition') ?>>
                                        REQUISITION
                                    </option>
                                    <option value="vendor"<?= set_select('sort_by', 'vendor', get_url_param('sort_by') == 'vendor') ?>>
                                        VENDOR
                                    </option>
                                    <option value="total_item_quantity"<?= set_select('sort_by', 'total_item_quantity', get_url_param('sort_by') == 'total_item_quantity') ?>>
                                        TOTAL ITEM
                                    </option>
                                    <option value="total_price"<?= set_select('sort_by', 'total_price', get_url_param('sort_by') == 'total_price') ?>>
                                        TOTAL PRICE
                                    </option>
                                    <option value="lead_time"<?= set_select('sort_by', 'lead_time', get_url_param('sort_by') == 'lead_time') ?>>
                                        LEAD TIME
                                    </option>
                ...
                <div class="modal-footer">
                    <a href="<?= site_url('purchasing/offer') ?>" class="btn btn-sm btn-secondary">
                        RESET
                    </a>
