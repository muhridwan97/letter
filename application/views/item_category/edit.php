<form action="<?= site_url('master/item-category/update/' . $item['id']) ?>" method="POST" id="form-category">
    <?= _csrf() ?>
    <?= _method('put') ?>
        ...
        <div class="card-body">
                ...
                <select class="custom-select" id="category" name="category" required>
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= $category['id'] ?>"
                            <?= set_select('category', $category['id'], $category['id'] == $item['id_category']) ?>>
                            <?= $category['category'] ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="item_name" name="item_name" required maxlength="100"
                       value="<?= set_value('category', $item['item_name']) ?>" placeholder="Enter a item name">
                <?= form_error('category') ?>
            </div>
            <div class="form-group">
                        ...
                        <div class="form-check form-check-inline mt-2">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="is_reserved" id="is_reserved_yes" value="1"
                                    <?= set_radio('is_reserved', 1, $item['is_reserved'] == 1) ?> required>
                                YES
                        ...
                        <div class="form-check form-check-inline mt-2">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="is_reserved" id="is_reserved_no" value="0"
                                    <?= set_radio('is_reserved', 0, $item['is_reserved'] == 0) ?>>
                                NO
                                <i class="input-helper"></i>
                            </label>
                        </div>
